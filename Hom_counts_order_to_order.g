...
CrossOrderHomCounts := function(orderG, orderH, arg...)
  ...
  if outfile = true then
    outfile := Concatenation("Hom_counts_", String(orderG), "_to_", String(orderH), ".csv");
  fi;
  ...
  if outfile <> false then
    parts := [""];   # 第一列留空，用于行标签
    for j in [1..mH] do
      Add(parts, EscapeCSV(Concatenation( String(idsH[j][1]), ".", String(idsH[j][2]), " ", descH[j] )));
    od;
  ...
  # 双重循环计算
  for i in [1..mG] do
    parts := [EscapeCSV(Concatenation( String(idsG[i][1]), ".", String(idsG[i][2]), " ", descG[i] ))];
